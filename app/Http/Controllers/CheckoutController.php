...
class CheckoutController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($carts && count($carts) > 0) {
            ...
            if (get_setting('shipping_type') == 'carrier_wise_shipping') {
                ...
                $zone = getZoneIdFromShippingInfo($shipping_info);
    ...
    public function checkout(Request $request)
    {
        if ($request->payment_option == null) {
            ...
            return redirect()->route('checkout.single_page_checkout');
        }
        ...
        // Update shipping costs based on selected delivery option before creating order
        // Check both camelCase and snake_case for delivery option
        $deliveryOption = $request->input('deliveryOption') ?? $request->input('delivery_option');
        
        if ($deliveryOption) {
            $zone = null;
            $zoneName = null;
            
            // Check if it's in the new format (zone_{id})
            if (strpos($deliveryOption, 'zone_') === 0) {
                $zoneId = str_replace('zone_', '', $deliveryOption);
            ...
            elseif ($deliveryOption === 'insideDhaka') {
                $zoneName = 'Inside Dhaka';
            } elseif ($deliveryOption === 'outsideDhaka') {
            ...
            $shipping_cost = 0;
            if ($zone) {
                $shipping_cost = CarrierRangePrice::leftJoin('zones', 'zones.id', 'carrier_range_prices.zone_id')
                    ->where('carrier_range_prices.zone_id', $zone->id)
                    ->where('zones.status', 1)
                    ->value('carrier_range_prices.price') ?? 0;
            } elseif ($zoneName) {
                $shipping_cost = CarrierRangePrice::leftJoin('zones', 'zones.id', 'carrier_range_prices.zone_id')
                    ->where('zones.name', $zoneName)
                    ->where('zones.status', 1)
                    ->value('carrier_range_prices.price') ?? 0;
            }
            
            // Update shipping costs for all cart items
            $total_items = $carts->count();
            if ($total_items > 0 && $shipping_cost > 0) {
                $shipping_cost_per_item = $shipping_cost / $total_items;
                foreach ($carts as $cart) {
                    $cart->shipping_cost = $shipping_cost_per_item;
        ...
        if (!auth()->check()) {
            if (!$request->has('name') || !$request->has('phone') || !$request->has('address')) {
                flash(translate('Please fill in all required shipping information.'))->warning();
                return redirect()->route('checkout.single_page_checkout');
            }
        }
    ...
    public function single_page_checkout(Request $request){
        ...
        if ($carts && count($carts) > 0) {
            ...
            if (Auth::check()) {
                $defaultAddress = Auth::user()->addresses->where('set_default', 1)->first();
            }
            
            // Get all active zones (both regular zones and sub-zones)
            $zones = Zone::where('status', 1)->with(['parentZone', 'areas.city'])->get();
            
            return view('frontend.payment_select_new', compact('carts','total','defaultAddress','zones'));
    ...
    public function shipment(Request $request)
    {
        ...
        // Check if delivery option is manually selected (new format: zone_{id})
        if ($request->has('delivery_option')) {
            ...
            if (strpos($deliveryOption, 'zone_') === 0) {
                ...
                if ($zone) {
                    $zoneName = $zone->name;
                    // Check if it's a sub-zone (has parent_zone_id)
                    $isSubZone = $zone->parent_zone_id !== null;
                    // Use the selected zone directly (both regular zones and sub-zones have their own shipping costs)
                }
            } 
            ...
            // Fallback to division/district logic if delivery_option not provided
            $district = District::where('id', $request->input('district_id'))->value('name');

            if ($request->has('division_id')) {
                $division = Division::with('zone')->find($request->input('division_id'));
                if ($division && $division->zone) {
                    $zone = $division->zone;
                    $zoneName = $zone->name;
                    if (strtolower($zoneName) === 'inside dhaka') {
                        $isInsideDhaka = true;
                    }
                    $isSubZone = $zone->parent_zone_id !== null;
                }
        ...
        if ($zone) {
            $shipping_cost = CarrierRangePrice::leftJoin('zones', 'zones.id', 'carrier_range_prices.zone_id')
                ->where('carrier_range_prices.zone_id', $zone->id)
                ->where('zones.status', 1)
                ->value('carrier_range_prices.price');
        } elseif ($zoneName) {
            $shipping_cost = CarrierRangePrice::leftJoin('zones', 'zones.id', 'carrier_range_prices.zone_id')
                ->where('zones.name', $zoneName)
                ->where('zones.status', 1)
        ...
        foreach ($carts as $cart) {
            $cart['shipping_cost'] = $shipping_cost / max(1, $total_item);
            $cart->save();
        }

        $coupon = '';
        $returnHTML = view('frontend.partials.cart_summary', compact('coupon', 'carts'))->render();

        // Determine if it's "Outside Dhaka" zone (by name)
        $isOutsideDhakaZone = false;
        if ($zoneName) {
            $isOutsideDhakaZone = (strtolower(trim($zoneName)) === 'outside dhaka');
        }
        ...
        return response()->json([
            ...
            'is_outside_dhaka_zone' => $isOutsideDhakaZone
        ]);
    }
    
    public function store_shipping_payment(Request $request)
    {
        $amount = $request->input('amount');
        $delivery_option = $request->input('delivery_option');
        
        // Store shipping payment info in session
        $request->session()->put('shipping_fee_payment', [
            'amount' => $amount,
        ...
        // Store order form data in session for auto-completion after payment
        $orderFormData = $request->except(['amount', 'delivery_option', '_token']);
        $request->session()->put('pending_order_data', $orderFormData);
        ...
        $request->session()->put('payment_type', 'shipping_fee_payment');
        $request->session()->put('payment_data', $payment_data);
        
        // Use Bkash controller to get payment URL
        $decorator = __NAMESPACE__ . '\\Payment\\BkashController';
        if (class_exists($decorator)) {
            $bkashController = new $decorator;
            
            // Get token and amount
            Session::forget('bkash_token');
            Session::put('bkash_token', $bkashController->getToken());
            ...
            if (isset($result->bkashURL)) {
                ...
                return response()->json([
                    'success' => false,
                    'message' => translate('Failed to initialize Bkash payment. Please try again.')
                ]);
            }
        } else {
            return response()->json([
                'success' => false,
                'message' => translate('Bkash payment gateway is not available')
            ]);
        }
    }
    
    public function shipping_fee_payment_done($payment_data, $payment_details)
    {
        // Mark shipping fee as paid in session
        Session::put('shipping_fee_payment.status', 'paid');
        Session::put('shipping_fee_payment.payment_details', $payment_details);
        
        // Get stored order form data
        $pendingOrderData = Session::get('pending_order_data');
        
        if ($pendingOrderData) {
            // Create a request object from stored data
            $pendingOrderData['payment_option'] = 'cash_on_delivery'; // Set COD as payment method
            $request = Request::create(route('payment.checkout'), 'POST', $pendingOrderData);
            
            // Set the session on the request object so OrderController can use it
            $request->setLaravelSession(app('session.store'));
            
            // Handle both authenticated and guest users
            if (auth()->check()) {
                $user = auth()->user();
                $carts = \App\Models\Cart::where('user_id', $user->id)->active()->get();
            } else {
                $temp_user_id = Session::get('temp_user_id');
                $carts = ($temp_user_id != null) ? \App\Models\Cart::where('temp_user_id', $temp_user_id)->active()->get() : collect();
            }
            
            if ($carts->isNotEmpty()) {
                // Create the order
                (new \App\Http\Controllers\OrderController)->store($request);
                
                // Get the combined order ID
                $combined_order_id = Session::get('combined_order_id');
                
                if ($combined_order_id) {
                    // Mark shipping fee payment in order
                    $combined_order = \App\Models\CombinedOrder::find($combined_order_id);
                    if ($combined_order) {
                        foreach ($combined_order->orders as $order) {
                            // Store shipping fee payment details in order
                            $orderPaymentDetails = json_decode($order->payment_details, true) ?? [];
                            $orderPaymentDetails['shipping_fee_paid'] = true;
                            $orderPaymentDetails['shipping_fee_payment_details'] = $payment_details;
                            $order->payment_details = json_encode($orderPaymentDetails);
                            $order->save();
                        }
                    }
        ...
        // If auto-completion failed, redirect back to checkout page with success parameter
        return redirect()->route('checkout.single_page_checkout', ['shipping_payment' => 'success']);
    }
    ...
    public function checkout_done1($combined_order_id, $payment)
    {
        ...
        foreach ($combined_order->orders as $key => $order) {
            ...
            // Order paid notification to Customer, Seller, & Admin (skip for guest orders)
    ...
    public function order_confirmed()
    {
        ...
        $combined_order = CombinedOrder::with([
            ...
            'orders' => function($query) {
                ...
                ])->distinct(); // Ensure no duplicate orders
            }
        ])->find($combined_order_id);
        ...
        if (!$combined_order->orders || $combined_order->orders->isEmpty()) {
            flash(translate('No orders found'))->error();
            return redirect()->route('home');
        }

...
class OTPController extends Controller
{
    ...
    {
        // Ensure bdbulksms configuration exists in database
        $bdbulksms = OtpConfiguration::where('type', 'bdbulksms')->first();
        if (!$bdbulksms) {
            $bdbulksms = new OtpConfiguration;
            $bdbulksms->type = 'bdbulksms';
            $bdbulksms->value = 0; // Inactive by default
            $bdbulksms->save();
        }

...
class BusinessSettingsController extends Controller
{
    public function __construct()
    {
        ...
        $this->middleware(['permission:analytics_tools_configuration'])->only(['google_analytics', 'google_tag_manager_update']);
    ...
    public function shipping_configuration_update(Request $request)
    {
        if ($request->type == 'shipping_type' && $request->shipping_type == 'carrier_wise_shipping') {
            ...
            $hasInvalidDivisions = Division::where('status', 1)
            ...
            if ($hasInvalidDivisions) {
                flash(translate('Your active shipping divisions are assigned to inactive or undefined shipping zones. Please review your zone setup before enabling carrier-wise shipping.'))->error();
                return back();
            }
        }
        $business_settings = BusinessSetting::where('type', $request->type)->first();

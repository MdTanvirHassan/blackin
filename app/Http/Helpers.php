...
if (!function_exists('home_base_price_by_stock_id')) {
    function home_base_price_by_stock_id($id, $formatted = true)
    {
        ...
        return $formatted ? format_price($converted) : $converted;
    }
}
...
if (!function_exists('home_discounted_base_price_by_stock_id')) {
    function home_discounted_base_price_by_stock_id($id, $formatted = true)
    {
        $product_stock = ProductStock::findOrFail($id);
...
/**
 * Get zone ID from shipping info
 * Tries to get zone from division, falls back to country for backward compatibility
 */
if (!function_exists('getZoneIdFromShippingInfo')) {
    function getZoneIdFromShippingInfo($shipping_info)
    {
        // Try to get zone from division_id if available
        if (isset($shipping_info['division_id']) && $shipping_info['division_id'] != 0) {
            $division = Division::where('id', $shipping_info['division_id'])->where('status', 1)->first();
            if ($division && $division->zone_id) {
                return $division->zone_id;
            }
        }
        
        // Try to get zone from district_id if available
        if (isset($shipping_info['district_id']) && $shipping_info['district_id'] != 0) {
            $district = District::where('id', $shipping_info['district_id'])->first();
            if ($district && $district->division_id) {
                $division = Division::where('id', $district->division_id)->where('status', 1)->first();
                if ($division && $division->zone_id) {
                    return $division->zone_id;
                }
            }
        ...
        // Try to get zone from city->district->division if city_id is available
        if (isset($shipping_info['city_id']) && $shipping_info['city_id'] != 0) {
            $city = City::where('id', $shipping_info['city_id'])->first();
            if ($city && isset($city->district_id) && $city->district_id != 0) {
                $district = District::where('id', $city->district_id)->first();
                if ($district && $district->division_id) {
                    $division = Division::where('id', $district->division_id)->where('status', 1)->first();
                    if ($division && $division->zone_id) {
                        return $division->zone_id;
        ...
        // Fall back to country->zone_id for backward compatibility
        if (isset($shipping_info['country_id']) && $shipping_info['country_id'] != 0) {
            $country = Country::where('id', $shipping_info['country_id'])->where('status', 1)->first();
            if ($country && $country->zone_id) {
...
function getShippingCost($carts, $index, $shipping_info = '', $carrier = '')
{
    ...
    if ($shipping_type == 'flat_rate') {
        ...
        $user_zone = getZoneIdFromShippingInfo($shipping_info);
...
if (!function_exists('get_element_type_by_id')) {
    ...
    {
        if (!Schema::hasTable('element_types') || empty($id)) {
            return null;
        }
...
if (!function_exists('get_element_style_value')) {
    ...
    {
        if (!Schema::hasTable('element_styles') || empty($element_type_id)) {
            return null;
        }

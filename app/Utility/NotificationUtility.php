...
class NotificationUtility
{
    ...
    {       
        ...
        // Skip SMS notifications for guest orders
        if ($order->user_id != null && addon_is_activated('otp_system') && SmsTemplate::where('identifier', 'order_placement')->first()->status == 1) {
            try {
        ...
        //sends Notifications to user (skip for guest orders)
        if ($order->user_id != null) {
    ...
    {     
        ...
        $adminId = get_admin()->id;
        $userIds = array($order->user_id, $order->seller_id);
        if ($order->seller_id != $adminId) {
            array_push($userIds, $adminId);
        }
        $users = User::findMany($userIds);

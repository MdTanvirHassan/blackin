                    ...
                    <table class="table aiz-table mb-0">
                        <thead>
                            <tr>
                                ...
                                <th>{{ translate('Parent Zone') }}</th>
                                <th>{{ translate('Divisions/Areas') }}</th> 
                            ...
                            @foreach ($zones as $key => $zone)
                                <tr>
                                    ...
                                    <td>
                                        {{ $zone->name }}
                                        @if($zone->isSubZone())
                                            <span style="width: 100px; text-align: center;" class="badge badge-info">{{ translate('Sub Zone') }}</span>
                                        @endif
                                    </td> 
                                    <td>
                                        @if($zone->isSubZone())
                                            <span style="width: 100px; text-align: center;" class="badge badge-info">{{ translate('Sub Zone') }}</span>
                                        @else
                                            <span style="width: 100px; text-align: center;" class="badge badge-primary">{{ translate('Regular Zone') }}</span>
                                        @endif
                                    </td>
                                    <td>
                                        @if($zone->parentZone)
                                    ...
                                    <td>
                                        @if($zone->isSubZone())
                                            ...
                                            @endphp
                                            @if($areas->count() > 0)
                                                {{ $areas->pluck('name')->take(3)->join(', ') }}
                                                @if($areas->count() > 3)
                                                    <span class="text-muted">(+{{ $areas->count() - 3 }} {{ translate('more') }})</span>
                                                @endif
                                            @else
                                                <span class="text-muted">-</span>
                                            @endif
                                        @else
                                            ...
                                            @if($divisions->count() > 0)
                                                {{ $divisions->pluck('name')->take(3)->join(', ') }}
                                                @if($divisions->count() > 3)
                                                    <span class="text-muted">(+{{ $divisions->count() - 3 }} {{ translate('more') }})</span>
                                                @endif
                                            @else
                                                <span class="text-muted">-</span>
                                            @endif

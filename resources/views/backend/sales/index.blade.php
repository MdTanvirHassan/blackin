                ...
                <div class="col-lg-3">
                    <div class="form-group mb-0">
                        <input type="text" class="form-control" id="search"
                            ...
                            placeholder="{{ translate('Type Order code, Customer name or phone & hit Enter') }}">
                    </div>
                </div>
                <div class="col-auto">
                    <div class="form-group mb-0">
                ...
                <table class="table aiz-table mb-0">
                    <thead>
                        <tr>
                            ...
                            <th data-breakpoints="md">{{ translate('Customer Type') }}</th>
                            <th data-breakpoints="md">{{ translate('Customer') }}</th>
                            <th data-breakpoints="md">{{ translate('Amount') }}</th>
                            <th data-breakpoints="md">{{ translate('Delivery Status') }}</th>
                            <th data-breakpoints="md">{{ translate('Payment method') }}</th>
                            <th data-breakpoints="md">{{ translate('Payment Status') }}</th>
                            <th data-breakpoints="lg">{{ translate('Shipping Fee Payment') }}</th>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @if ($order->user != null)
                                        <span class="badge badge-inline badge-success">
                                            <i class="las la-check-circle"></i> {{ translate('Registered') }}
                                        </span>
                                    @else
                                        <span class="badge badge-inline badge-warning">
                                            <i class="las la-exclamation-circle"></i> {{ translate('Guest') }}
                                        </span>
                                    @endif
                                </td>
                                <td>
                                    ...
                                    @else
                                        @php
                                            ...
                                            $guestName = $shippingAddress['name'] ?? 'Guest';
                                        @endphp
                                        <span>
                                            <i class="las la-user-secret"></i> {{ $guestName }}
                                        </span>
                                    @endif
                                </td>
                                ...
                                <td>
                                    @php
                                        ...
                                        $statusIcons = [
                                            ...
                                            'confirmed' => 'la-check',
                                            'picked_up' => 'la-box',
                                            'on_the_way' => 'la-shipping-fast',
                                            'delivered' => 'la-check-circle',
                                            'cancelled' => 'la-times-circle'
                                        ];
                                        $color = $statusColors[$order->delivery_status] ?? 'secondary';
                                        $icon = $statusIcons[$order->delivery_status] ?? 'la-info-circle';
                                    @endphp
                                    <span class="badge badge-inline badge-{{ $color }}">
                                        <i class="las {{ $icon }}"></i> {{ translate(ucfirst(str_replace('_', ' ', $order->delivery_status))) }}
                                    </span>
                                </td>
                                <td>
                                ...
                                <td>
                                    @php
                                        ...
                                        $shippingFeePaid = isset($paymentDetails['shipping_fee_paid']) && $paymentDetails['shipping_fee_paid'];
                                        $shippingFeePaymentDetails = $paymentDetails['shipping_fee_payment_details'] ?? null;
                                    @endphp
                                    @if ($shippingFeePaid && $shippingFeePaymentDetails)
                                        @php
                                            $bkashDetails = is_string($shippingFeePaymentDetails) ? json_decode($shippingFeePaymentDetails, true) : $shippingFeePaymentDetails;
                                            $trxId = $bkashDetails['trxID'] ?? $bkashDetails['transactionID'] ?? 'N/A';
                                            $amount = $bkashDetails['amount'] ?? 'N/A';
                                        @endphp
                                        <span style="width: auto;" class="badge badge-inline badge-success" title="{{ translate('Payment ID') }}: {{ $trxId }}">
                                            <i class="las la-check-circle"></i> {{ translate('Paid via bKash') }}
                                        </span>

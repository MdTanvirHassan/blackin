            ...
            <div class="row gutters-5">
                <div class="col text-md-left text-center">
                    ...
                    @if($shipping_address)
                        <address>
                            <strong class="text-main">
                                {{ $shipping_address->name ?? ($order->user ? $order->user->name : translate('N/A')) }}
                            </strong><br>
                            {{ $shipping_address->email ?? ($order->user ? $order->user->email : '') }}<br>
                            {{ $shipping_address->phone ?? ($order->user ? $order->user->phone : '') }}<br>
                    ...
                    @else
                        <address>
                            <strong class="text-main">
                                {{ $order->user ? $order->user->name : translate('N/A') }}
                            </strong><br>
                            @if($order->user)
                                {{ $order->user->email }}<br>
                                {{ $order->user->phone ?? '' }}<br>
                        ...
                        <tbody>
                            ...
                            @foreach ($groupedDetails as $hash => $group)
                                @php
                                    ...
                                    $groupTotalQuantity = $groupItems->sum('quantity');
                                @endphp
                                
                                {{-- Group Product Header --}}
                                <tr class="bg-light">
                                    <td class="fw-600">{{ ++$itemCounter }}</td>
                                    ...
                                    <td class="text-center fw-600">{{ $groupTotalQuantity }}</td>
                                    <td class="text-center fw-600">-</td>
                                    <td class="text-center fw-600">{{ single_price($groupTotalPrice) }}</td>
                                </tr>

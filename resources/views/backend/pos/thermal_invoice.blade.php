			...
			<div style="border-bottom:1px dotted #9c9ca1;padding-bottom: 0.25rem;">
				...
				<table>	
					@php
						...
						if (is_string($shipping_address_raw)) {
							$shipping_address = json_decode($shipping_address_raw, true);
						} else {
							$shipping_address = is_array($shipping_address_raw) ? $shipping_address_raw : (array) $shipping_address_raw;
						}
					...
					<tr><td class="gry-color small">{{ translate('Address') }}: {{ ($shipping_address['address'] ?? '') }}, {{ ($shipping_address['city'] ?? '') }},  @if(isset($shipping_address['state'])) {{ $shipping_address['state'] }} - @endif {{ ($shipping_address['postal_code'] ?? '') }}, {{ ($shipping_address['country'] ?? '') }}</small></td></tr>
					<tr><td class="gry-color small">{{ translate('Email') }}: {{ $shipping_address['email'] ?? ($order->user->email ?? '') }}</small></td></tr>

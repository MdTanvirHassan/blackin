            ...
            <div class="p-4 border h-100">
                ...
                @if(Auth::user()->addresses != null)
                    ...
                    <ul class="list-unstyled mb-3">
                        <li class="fs-14 fw-400 text-dark pb-1">{{ $address->address }}</li>
                        <li class="fs-14 fw-400 text-dark pb-1">{{ $address->postal_code }}</li>
                        @if($address->division)
                        <li class="fs-14 fw-400 text-dark pb-1">{{ $address->division->getTranslation('name') }}</li>
                        @endif
                        @if($address->district)
                        <li class="fs-14 fw-400 text-dark pb-1">{{ $address->district->getTranslation('name') }}</li>
                        @endif
                        <li class="fs-14 fw-400 text-dark pb-1">{{ $address->country?->name }}</li>
                        <li class="fs-14 fw-400 text-dark pb-1">{{ $address->phone }}</li>
                    </ul>
                    @endif

                    <!-- View All Addresses Button -->
                    <button class="btn btn-primary btn-sm mb-3" type="button" data-toggle="collapse" data-target="#all-addresses" aria-expanded="false">
                        {{ translate('View All Addresses') }}
                    </button>

                    <!-- All Addresses Collapse -->
                    <div class="collapse" id="all-addresses">
                    @foreach(Auth::user()->addresses->filter(function ($address) {
                        return $address->set_default == 0;
                    }) as $addr)
                            <div class="border-top pt-2 mt-2">
                                @if (!empty($addr->name))
                                    <p class="mb-1 fw-600">{{ $addr->name }}</p>
                                @endif

                                <p class="mb-1">
                                    <strong>{{ $addr->set_default ? '🌟 ' : '' }}</strong>{{ $addr->address }}
                                </p>

                                <p class="mb-1">
                                    {{ $addr->postal_code }}
                                    @if($addr->division)
                                        , {{ $addr->division->getTranslation('name') }}
                                    @endif
                                    @if($addr->district)
                                        , {{ $addr->district->getTranslation('name') }}
                                    @endif
                                    , {{ $addr->country?->name }}
                                </p>
                                ...
                                @if (!$addr->set_default)
                                    <form method="POST" action="{{ route('addresses.make_default', $addr->id) }}">
                                        @csrf
                                ...
                                @else
                                    <span class="badge badge-success mt-2">{{ translate('Default') }}</span>
                                @endif
                            </div>
                        @endforeach
                    </div>
                ...
                <button class="btn btn-dark btn-block fs-14 fw-500 mt-3" onclick="add_new_address()" style="border-radius: 25px;">
                    <i class="la la-plus fs-18 fw-700 mr-2"></i>

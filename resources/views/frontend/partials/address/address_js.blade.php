<script type="text/javascript">
    ...
    $(document).on('change', '[name=division_id]', function() {
        var division_id = $(this).val();
        get_districts(division_id);
    });
    ...
    function get_districts(division_id) {
        ...
        if (!division_id) {
            $('[name="district_id"]').html('<option value="">{{ translate('Select District') }}</option>');
            AIZ.plugins.bootstrapSelect('refresh');
            return;
        }
        $.ajax({
            ...
            success: function (response) {
                var obj = JSON.parse(response);
                if(obj != '' && $('<select></select>').html(obj).find('option').length > 1) {
                    $('[name="district_id"]').attr('disabled', false);
                    $('[name="district_id"]').html(obj);
                    AIZ.plugins.bootstrapSelect('refresh');
                }else{
                    $('[name="district_id"]').html('<option value="">{{ translate('No districts are available under this division.') }}</option>');
                    $('[name="district_id"]').attr('disabled', true);
                    AIZ.plugins.bootstrapSelect('refresh');
                }
            }
        });

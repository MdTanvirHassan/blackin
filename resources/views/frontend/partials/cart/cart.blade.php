@php
    $total = 0;
    // Check if group_carts are passed (from AJAX for group products) or use get_user_cart()
    if (isset($group_carts)) {
        $carts = $group_carts;
    } else {
        $carts = get_user_cart();
    }
    ...
    if (is_object($carts) && method_exists($carts, 'count')) {
        $cartsArray = [];
        foreach ($carts as $cart) {
            if (is_object($cart)) {
                $cartsArray[] = [
                    ...
                    'group_product_id' => isset($cart->group_product_id) ? $cart->group_product_id : (isset($cart['group_product_id']) ? $cart['group_product_id'] : null),
                    'group_product_slot_id' => isset($cart->group_product_slot_id) ? $cart->group_product_slot_id : (isset($cart['group_product_slot_id']) ? $cart['group_product_slot_id'] : null),
                    'group_product_slot_combination_hash' => isset($cart->group_product_slot_combination_hash) ? $cart->group_product_slot_combination_hash : (isset($cart['group_product_slot_combination_hash']) ? $cart['group_product_slot_combination_hash'] : null),
                ];
            } else {
                $cartsArray[] = $cart;
            }
    ...
    foreach ($carts as $cartItem) {
        $groupId = isset($cartItem['group_product_id']) ? $cartItem['group_product_id'] : (isset($cartItem->group_product_id) ? $cartItem->group_product_id : null);
        $slotHash = isset($cartItem['group_product_slot_combination_hash']) ? $cartItem['group_product_slot_combination_hash'] : (isset($cartItem->group_product_slot_combination_hash) ? $cartItem->group_product_slot_combination_hash : null);
        ...
        if ($groupId) {
            ...
            $itemQuantity = isset($cartItem['quantity']) ? $cartItem['quantity'] : (isset($cartItem->quantity) ? $cartItem->quantity : 0);
            $groupedCarts[$groupKey]['items'][] = $cartItem;
            $groupedCarts[$groupKey]['total_price'] += $itemPrice * $itemQuantity;
            $groupedCarts[$groupKey]['quantity'] = max($groupedCarts[$groupKey]['quantity'], $itemQuantity); // Use max quantity as bundle quantity
        } else {
            $ungroupedCarts[] = $cartItem;
        }
    }
    
    // Calculate total
    foreach ($carts as $key => $cartItem) {
        $productId = isset($cartItem['product_id']) ? $cartItem['product_id'] : (isset($cartItem->product_id) ? $cartItem->product_id : null);
        if ($productId) {
            $product = get_single_product($productId);
            if ($product) {
                $itemPrice = isset($cartItem['price']) ? $cartItem['price'] : (isset($cartItem->price) ? $cartItem->price : cart_product_price($cartItem, $product, false));
                $itemQuantity = isset($cartItem['quantity']) ? $cartItem['quantity'] : (isset($cartItem->quantity) ? $cartItem->quantity : 0);
...
<a href="javascript:void(0)" class="d-flex align-items-center p-0 m-0 utility-nav-icon" data-toggle="dropdown" data-display="static"
    title="{{translate('Cart')}}">
    <span class="mr-0">
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="20.562" viewBox="0 0 24 20.562"
            class="utility-icon-svg {{ get_setting('header_element') !=5 ? 'bottom-text-color-visibility' : 'middle-text-color-visibility' }}"
    ...
    <!-- <span class="d-none d-xl-block ml-0 fs-14 fw-700 {{ get_setting('header_element') !=5 ? 'bottom-text-color-visibility' : 'middle-text-color-visibility' }}"
    ...
    <span class="nav-box-text d-none d-xl-block ml-0 fs-12 {{ get_setting('header_element') !=5 ? 'bottom-text-color-visibility' : 'middle-text-color-visibility' }}"
        style="color: {{ get_setting('header_element') !=5 ? get_setting('bottom_header_text_color'): get_setting('middle_header_text_color') }}">
...
<!-- Cart Items -->
<div id="cart_items" class="dropdown-menu dropdown-menu-right dropdown-menu-lg p-0 stop-propagation rounded-0" style="min-width: 500px; max-width: 1000px;">
    @if (count($carts) > 0)
        ...
        <ul class="h-360px overflow-auto c-scrollbar-light list-group list-group-flush mx-1">
            ...
            @foreach ($groupedCarts as $groupKey => $groupData)
                @php
                    $groupId = $groupData['group_product_id'];
                    $groupProduct = \App\Models\GroupProduct::find($groupId);
                    $firstItem = $groupData['items'][0];
                @endphp
                @if ($groupProduct)
                    <li class="list-group-item border-0 p-3 mb-2" style="background: #f8f9fa; border-left: 3px solid #007bff !important;">
                        <!-- Bundle Header -->
                        <div class="d-flex align-items-start mb-2">
                            <img src="{{ static_asset('assets/img/placeholder.jpg') }}" 
                                 data-src="{{ uploaded_asset($groupProduct->thumbnail_img) }}"
                                 class="lazyload img-fit rounded mr-3" 
                                 style="width: 60px; height: 60px; object-fit: cover;"
                                 alt="{{ $groupProduct->name }}" 
                                 onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder.jpg') }}';">
                            <div class="flex-grow-1">
                                <h6 class="fs-13 fw-700 mb-1 text-dark text-truncate-2">
                                    {{ $groupProduct->name }}
                                </h6>
                                <div class="fs-12 fw-400 text-secondary mb-1">
                                    {{ translate('Quantity')}}: {{ $groupData['quantity'] }} × {{ translate('Bundle') }}
                                </div>
                                <div class="fs-14 fw-700 text-primary">
                                    {{ single_price($groupData['total_price']) }}
                                </div>
                            </div>
                            <button onclick="removeGroupProductFromCart('{{ $groupKey }}')"
                                class="btn btn-sm btn-icon stop-propagation ml-2">
                                <i class="la la-close fs-18 fw-600 text-secondary"></i>
                            </button>
                        </div>
                        <!-- Bundle Items -->
                        <div class="bundle-items-list mt-2">
                            @foreach($groupData['items'] as $cartItem)
                                @php
                                    $itemProductId = isset($cartItem['product_id']) ? $cartItem['product_id'] : (isset($cartItem->product_id) ? $cartItem->product_id : null);
                                    $itemProduct = $itemProductId ? get_single_product($itemProductId) : null;
                                    
                                    // Parse variation to format as "Color: Darkcyan Size: L"
                                    $variationDisplay = '';
                                    if (isset($cartItem['variation']) && $cartItem['variation']) {
                                        $variation = $cartItem['variation'];
                                        $choiceOptions = json_decode($itemProduct->choice_options ?? '[]', true) ?? [];
                                ...
                                @endphp
                                @if ($itemProduct)
                                    <div class="bundle-item-card d-flex align-items-start p-2 mb-1 bg-white rounded" style="border-left: 2px solid #dee2e6;">
                                        ...
                                        <div class="flex-grow-1">
                                            <div class="d-flex justify-content-between align-items-start">
                                                <div class="flex-grow-1">
                                                    <h6 class="fs-11 fw-600 mb-0 text-dark" style="line-height: 1.3;">
                                                        {{ $itemProduct->getTranslation('name') }}
                                                    </h6>
                                                    @if($variationDisplay)
                                                        <span class="fs-10 text-muted d-block">
                                                            {{ $variationDisplay }}
                                                        </span>
                                                    @endif
                                                </div>
                                                <div class="text-right ml-2">
                                                    <div class="fs-12 fw-700 text-primary">
                                                        {{ single_price((isset($cartItem['price']) ? $cartItem['price'] : 0) * (isset($cartItem['quantity']) ? $cartItem['quantity'] : 0)) }}
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
            ...
            @foreach ($ungroupedCarts as $key => $cartItem)
                @php
                    $productId = isset($cartItem['product_id']) ? $cartItem['product_id'] : (isset($cartItem->product_id) ? $cartItem->product_id : null);
                    $product = $productId ? get_single_product($productId) : null;
                @endphp
                @if ($product != null)
                        ...
                        <span class="d-flex align-items-center">
                            <a href="{{ route('product', $product->slug) }}"
                                ...
                                <span class="minw-0 pl-2 flex-grow-1">
                                    ...
                                    <span class="fs-14 fw-400 text-secondary">{{ isset($cartItem['quantity']) ? $cartItem['quantity'] : $cartItem->quantity }}x</span>
                                    <span class="fs-14 fw-400 text-secondary">{{ single_price(isset($cartItem['price']) ? $cartItem['price'] : (isset($cartItem->price) ? $cartItem->price : cart_product_price($cartItem, $product))) }}</span>
                                </span>
                            </a>
                            <span class="">
                                <button onclick="removeFromCart({{ isset($cartItem['id']) ? $cartItem['id'] : $cartItem->id }})"

...
<div class="aiz-card-box h-auto bg-white p-1 hov-scale-img border-none rounded-lg shadow-sm hove-shadow-md product-box-hover" style="transition: background-color 0.3s ease;">
    <div class="position-relative h-140px h-md-200px img-cover overflow-hidden">
        @php
            ...
            $seller = $product->user ? $product->user->name : translate('Unknown');
            $total_sales = $product->num_of_sale ?? 0;
            $rating = round($product->reviews->avg('rating'), 1);
        @endphp
        <!-- Image -->
        <a href="{{ $product_url }}" class="d-block bg-light rounded-lg p-0 h-100">
        ...
        @if (discount_in_percentage($product) > 0)
            <span class="absolute-top-left bg-primary ml-1 mt-1 fs-11 fw-700 text-white w-35px text-center rounded-pill shadow-sm"
        ...
        @if ($product->auction_product == 0)
            ...
            <div class="absolute-top-right aiz-p-hov-icon">
                <a href="javascript:void(0)" class="hov-svg-white rounded-circle" onclick="addToWishList({{ $product->id }})"
                ...
                <a href="javascript:void(0)" class="hov-svg-white rounded-circle" onclick="addToCompare({{ $product->id }})"
        ...
        <div class="fs-14 d-flex  mt-1">
            ...
            @if ($product->auction_product == 0)
                ...
                @if (home_base_price($product) != home_discounted_base_price($product))
                    <div class="disc-amounts has-transition">
    ...
    <!-- Total Sales & Rating -->
    {{-- <div class="flex items-center justify-center space-x-3 text-xs text-gray-500 mb-2">
            <span><i class="las la-shopping-bag text-primary"></i> {{ $total_sales }} {{ translate('Sold') }}</span>
            <span><i class="las la-star text-yellow-400"></i> {{ $rating }}/5</span>
        </div> --}}

        <!-- Shop Name -->
        {{-- <div class="p-1" >
            @if ($product->added_by == 'seller' && get_setting('vendor_system_activation') == 1)
                <span class="text-secondary fw-400 mr-4 w-80px" style="font-size: 10px!important;">{{ translate('Sold by') }}</span>
                <a href="{{ route('shop.visit', $product->user->shop->slug) }}"
                    class="text-reset hov-text-primary text-am fw-700">{{ $product->user->shop->name }}</a>
            @else
                <p class="mb-0 text-sm fw-700" style="font-size: 10px!important;">{{ translate('Inhouse product') }}</p>
            @endif
        </div> --}}

        <!-- add to cart -->
        <div class="d-flex justify-content-center py-2 border-top border-light">
            <a class="shadow-sm btn btn-sm btn-soft-dark-base btn btn-outline-dark hov-svg-white border border-dark border-1 hov-text-white rounded-4 d-flex align-items-center gap-2 @if (in_array($product->id, $cart_added)) active @endif"
                href="javascript:void(0)"
                onclick="showAddToCartModal({{ $product->id }})">
...
<style>
    .product-box-hover:hover {
        background-color: #f8f9fa !important; /* bg-light equivalent */
    }
</style>
